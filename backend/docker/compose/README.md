...
### Network
- **Name**: `retailmanager-net` (Docker bridge network)
...
docker exec postgres psql -U retailmanager -l
```
...
- `/docker/postgres/init/02-grant-privileges.sql`: Grants necessary permissions to retailmanager user
...
3. Test connectivity: `psql -h localhost -p 5544 -U retailmanager -l`
...
- **Credentials**: Currently using development defaults (retailmanager/RetailMgr2024! for Postgres, guest/guest for RabbitMQ)
